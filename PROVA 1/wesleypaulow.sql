...
DROP DATABASE WESLEYPAULOW;

CREATE DATABASE WESLEYPAULOW; 
...
CREATE TABLE professor(
    ...
    nome_Professor varchar(75) NOT NULL,
    sexo CHAR NOT NULL
);
...
INSERT INTO secao (fk_id_curso, titulo, explicacao)
VALUES
...
/* PARA INSERIR CHAVES NÃO É NECESSÁRIOS APÓSTROFE. */ 
...
UPDATE professor SET nome_professor = "Francisco", sexo = "M" WHERE id_professor = 6;
...
DELETE FROM curso WHERE nome_curso = "Python";
...
SELECT (nome_aluno) FROM aluno WHERE nome_aluno LIKE "%Silva%"; #like é o parâmetro para buscar strings que não são conhecidas por completo, junto do coringa % delimitando o local da palavra;
...
select (nome_aluno) as total_alunos from aluno where nascimento between'1990-01-01' and '2000-12-31';

# 5 - SELECIONAR QUANTOS ALUNOS ESTÃO INSCRITOS EM CADA UM DOS CURSOS E RENOMEIE AS COLUNAS DO SELECT. DEIXE O CODIGO ABAIXO:

select count(*) as total_alunos, fk_id_curso as curso FROM aluno_curso group by fk_id_curso;
...
select sum(nota) as soma_notas, avg(nota) as media_notas, min(nota) as menor_nota, max(nota) as maior_nota from nota;

# 7 - EXIBIR QUAIS OS TITULOS E AS EXPLICACÕES PARA OS TOPICOS DO CURSO DE "BANCO DE DADOS".

select titulo, explicacao from secao where fk_id_curso = 1;
...
select*from aluno where nascimento < '1999-12-31' order by nascimento desc, nome_aluno;